...
void addition_cl(const __fp16 *input, __fp16 *res,
  ...
  do {
    result = result =
      context.clCreateKernel(addition_cl_kernel_fp16_, context.LayerKernel::ADD_FP16,
    ...
    result = kernel_addition_fp16.SetKernelArguments(0, &inputA, sizeof(cl_mem));
    if (!result) {
      break;
    }

    result = kernel_addition_fp16.SetKernelArguments(1, &inOutRes, sizeof(cl_mem));
    if (!result) {
      break;
    }
